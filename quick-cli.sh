...
# shellcheck disable=SC2034
readonly R_PARAM_TOOL_VERSION_DESCRIPTION="要构建的工具版本"
...
action_params_check_create() {
  options_str="tool-version"
  check_action_param create tool-name
}
...
action_help() {
  ...
  printf "%s\n  %s\n\n" "版本:" "v1.0.0"
  printf "%s\n  %s\n\n" "Actions:" "help,create"
  explain_action "create" "创建cli" "./quick-cli.sh create --tool-name="lark-cli" --tool-version="v1.0.0""
}

## shell执行的核心部分
readonly R_LONG_OPTIONS="tool-name:,tool-version:,help"
readonly R_SHORT_OPTIONS="n:,v:,h"
readonly R_TOOL_ACTIONS="help,create"
if ! parameters=$(getopt -o "$R_SHORT_OPTIONS" -l "$R_LONG_OPTIONS" -n "$0" -- "$@"); then
  exit 1
fi
eval set -- "$parameters"
